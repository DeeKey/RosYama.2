...
<p><strong>Истек срок ожидания ответа от ГИБДД, на отправленные Вами запросы по барьерам:</strong></p>
<?php foreach ($holes as $i=>$hole) :?>
<?php $this->renderPartial("/holes/_view_in_mail", Array('data'=>$hole, 'index'=>$i, 'user'=>Yii::app()->user)); ?>
<?php endforeach; ?>
<p><strong>Если вы получили ответы на эти барьеры или они уже устранены, пожалуйста загрузите ответы либо отметьте факт исправления на сайте.</strong></p>
